...
- 第一局体验代理与 `tools/first-run-preview/shots/` **不是** Cocos 实机证据，更 **不是** Creator 3D 证据。本轮对症：程序化笔触/木纹像素肌理（grain/wash/burst）统一皮肤；市集挂鱼/钉眼/湿斑；湾鳍加鳞列/鳍骨/鳃；砸拍静帧用星爆裂纹扬尘加「砸！」；教学仍可自动甜区，**教完后自由局必须自己松手**。仍只能报代理分，不合 main。
