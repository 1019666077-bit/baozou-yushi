...
4. **应看到（3D 证据）：** 透视海面近/中/远分层 + 矮波；左侧码头栏杆/台阶/市集棚架；远处三岛分层剪影；日落侧光。HUD 仍是 2D 主橙 CTA。
5. 点「开始教学」出海：鱼是身/脸/鳞片/尾/大金弱点 5 件套，不是一条方块。抛竿镜头略跟线，命中微震，翻扑略抬镜。低配可关。
6. 建议截：港口全景、码头近景、湾鳍弱点、翻扑/砸甲板。期望构图（示意图，非实机）见 `docs/stage3d/`。
7. 云端没有 Creator，**不能**用代理截图冒充这 4 张。
