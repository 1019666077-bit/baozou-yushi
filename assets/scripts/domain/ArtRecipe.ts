...
export function huntWaterOps(look: IslandLook, phase = 0): DrawOp[] {
  ...
  const ops: DrawOp[] = [
    waveBandPoly(36, 6, 14, phase, rgba([255, 244, 214], 92), "foam"),
    waveBandPoly(12, -76, 18, phase + 0.28, rgba(mix(look.far, [210, 246, 255], 0.28), 112), "depth"),
    waveBandPoly(-64, -168, 22, phase + 0.82, rgba(mix(look.mid, look.near, 0.38), 118), "depth"),
    waveBandPoly(-154, -256, 24, phase + 1.18, rgba(mix(look.near, look.deep, 0.32), 124), "depth"),
    waveBandPoly(-242, -360, 18, phase + 1.72, rgba(look.deep, 148), "depth"),
    ...waveRidgeOps(26, 12, phase, rgba([255, 248, 230], 190), 5.4),
    ...waveRidgeOps(14, 10, phase + 0.18, rgba([10, 48, 72], 120), 4),
    ...waveRidgeOps(-20, 16, phase + 0.52, rgba([210, 246, 255], 150), 4.6),
    ...waveRidgeOps(-34, 14, phase + 0.68, rgba([8, 36, 58], 104), 3.6),
    ...waveRidgeOps(-92, 18, phase + 1.02, rgba([255, 248, 230], 128), 4.2),
    ...waveRidgeOps(-168, 20, phase + 1.42, rgba([170, 236, 255], 110), 3.8),
    ...waveRidgeOps(-244, 16, phase + 1.84, rgba([255, 244, 210], 90), 3.2),
