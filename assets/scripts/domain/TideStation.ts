...
/** 阿笠青绿上衣。 */
const TEAL: [number, number, number] = [42, 158, 126];
const TROUSER: [number, number, number] = [58, 78, 72];
const STRAW: [number, number, number] = [206, 176, 92];
const STRAW_DARK: [number, number, number] = [148, 124, 58];
const MINO: [number, number, number] = [168, 150, 78];
const MINO_DARK: [number, number, number] = [122, 104, 48];
...
/** 港口主角锁定：年轻版阿笠（斗笠蓑衣清爽钓手）。不是拾潮、不是港仔。 */
export const HARBOR_HERO = {
  id: "alisa",
  name: "阿笠",
  lock: "年轻版 A",
  look: "宽檐斗笠 + 厚蓑衣肩披 + 青绿上衣 + 钓竿",
} as const;
...
/** 年轻版阿笠：宽檐斗笠 + 多层蓑衣 + 青绿上衣 + 清楚钓竿。Primitives，无竞品。 */
