 ...
 * 不挡下一步点击：跟镜 ≤0.35s 量级，震幅小于 0.1 世界单位。
 */
...
export function camYankOffset(k: number): { x: number; y: number; z: number } {
  return {
...
export function camFlopOffset(airborne: boolean): { x: number; y: number; z: number } {
  if (!airborne) return { x: 0, y: 0, z: 0 };
  return { x: 0, y: CAM_FEEL.flopLiftY, z: -CAM_FEEL.flopPullZ };
}
...
export function composeHuntCam(input: {
  ...
  const flop = input.lowPower ? { x: 0, y: 0, z: 0 } : camFlopOffset(input.airborne === true);
  return {
