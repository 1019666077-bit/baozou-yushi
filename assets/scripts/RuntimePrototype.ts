...
export class RuntimePrototype extends Component {
  ...
  private resolveImpact(shot: Shot): void {
    ...
    SfxPlayer.play(
      judged.weakPoint ? "weak" : smashWindowOpen(smashGrade) ? "smash" : "hit",
    );
  ...
  private renderHud(): void {
    ...
    this.multiplier.string = `${styleGradeFor(style.points)}级 · ${comboHud(style.multiplier, style.combo, {
      rising: this.hudPunchLeft > 0,
      from: this.hudRiseFrom,
    })}${preview.captureChain > 0 ? ` · 高光链${preview.captureChain}` : ""}`;
    if (
      styleHudShouldPunch(
