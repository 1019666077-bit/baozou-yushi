...
**非 Cocos 实机。2D/辅助 ≠ Creator 3D 港湾/低模鱼，不能当真机手感 / 3D 画面证据。** 港口画面挂潮退浮站浏览器 3D 灰盒，猎场仍是 2D。
...
- 海景/船/鱼/码头/鱼箱跟 Runtime 自绘同一套休闲钓鱼皮肤。港口标题「海边鱼市」，背景是潮退浮站 3D 灰盒（浮台+阿笠+远景）；教学完成前不露码头差事。教完后码头差事是「钉块木板」（接差事 / 交木头），近岸可捞木头，码头 1→2 换皮。选岛条上提、渔具靠左码头，中屏让出海景。
...
- 买不起升级不抢港口主 CTA（与 `harborNextCta` 一致）；卖完主目标写还差多少（11/90）并提示再出海能补。
...
`serve.mjs` 会先跑 `extract-copy.mjs`，再 dump 潮退浮站零件到 `tools/tide-station-preview/generated/`（都不提交），港口 Three.js 从 `/tide-station/` 读同一套灰盒。
